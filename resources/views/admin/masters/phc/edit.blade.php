    ...
    <div class="container-fluid">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h4 class="text-themecolor">Edit Phc</h4>
            </div>
                ...
                <div class="card card-body">
                    ...
                    <form action="{{route('phc.update',$result->id)}}" enctype="multipart/form-data" method="post">
                        ...
                        <div class="row pt-3">
                            <div class="form-group col-sm-4 col-xs-4">
                                ...
                                <select name="is_urban" id="is_urban" class="form-control">
                                    @foreach($is_urban as $key => $value)
                                        <option value="{{$value}}" data-value="{{$value}}" {{SELECT($value,old('is_urban',$result->is_urban))}}>{{$key}}</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    @foreach($huds as $hud)
                                        <optgroup label="{{$hud->name}}">
                                            @foreach($hud->blocks as $block)
                                                <option value="{{$block->id}}" {{SELECT($block->id,old('block_id',$result->block_id))}}>{{$block->name}}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-sm-4 col-xs-4">
                                <label for="phc_image" class="">Image</label>
                                <input type="file" name="phc_image" class="form-control" id="phc_image" placeholder="Image" value="{{old('image_url',$result->image_url)}}" />
                            </div>
                            ...
                            <div class="row pt-3 col-md-12">
                                <div class="form-group col-sm-4 col-xs-4">
                                    <label for="image_url" class="required">PHC Image</label>
                                    @if($result->image_url)
                                        <br>
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    @if($result->property_document_url)
                                        ...
                                        <div class="position-relative mt-2">
                                                ...
                                                <br>
                                                <a href="{{ url('phc/destroy-document/'. $result->id) }}" onclick="return confirm('Are you sure you want to delete this document?')">
