                    ...
                    <form action="{{route('users.update',$result->id)}}" enctype="multipart/form-data" method="post">
                        ...
                        <div class="row pt-3">
                            ...
                            <div class="form-group col-sm-4 col-xs-4">
                                ...
                                <input type="text" name="username" class="form-control" id="username" placeholder="Enter UserName" value="{{old('username',$result->username)}}">
                            </div>
                            ...
                            @if($result->user_type_id != _hudUserTypeId())
                            <div class="form-group col-sm-4 col-xs-4">
                                <label for="section" class="required">Section</label>
                                <select name="section" id="section" class="form-control">
                                    @foreach($sections as $key => $value)
                                    <option value="{{$value->id}}" {{SELECT($value->id,old('section',$result->section))}}>{{$value->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            @endif
