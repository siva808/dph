@extends('admin.layouts.layout') @section('title', 'Edit Contact') @section('content') <div class="page-wrapper">
  <div class="container-fluid">
        ...
        <div class="d-flex justify-content-end align-items-center">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">
               <a href="{{route('contacts.index')}}">Contact</a></li>
            </li>
            @endif
          </ol>
        </div>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card card-body"> @if ($errors->any()) <div class="alert alert-danger">
            <ul> @foreach ($errors->all() as $error) <li>{{ $error }}</li> @endforeach </ul>
          </div> @endif <div class="row">
            <div class="col-sm-12 col-xs-12">
              <form action="{{route('contacts.update',$result->id)}}" enctype="multipart/form-data" method="post">
                {{csrf_field()}} @method('PUT')
                <input type="hidden" name="contact_type" id="contact_type" value="{{$result->contact_type}}" data-value="{{$result->contactType->slug_key}}">
                ...
                <div class="row pt-3">
                  @if(empty($isProfileUpdate))
                   <div class="form-group col-sm-4 col-xs-4" id="is_post_vacant_div">
                            ...
                            <select name="is_post_vacant" id="is_post_vacant" class="form-control">
                                ...
                                <option value="{{$key}}" data-value="{{$key}}" {{SELECT($key,old('is_post_vacant',$result->is_post_vacant))}}>{{$value}}</option>
                                @endforeach
                            </select>
                        </div>
                         @endif
                  ...
                  <div class="form-group col-sm-4 col-xs-4" id="designation_div">
                    ...
                    <select name="designation_id" id="designation_id" class="form-control">
                      <option value="">-- Select Designation -- </option> @foreach($designation as $key => $value) <option value="{{$value->id}}" {{SELECT($value->id,old('designation_id',$result->designation_id))}}>{{$value->name}}</option> @endforeach
                    </select>
                  </div>
                  ...
                  <div class="form-group col-sm-4 col-xs-4" id="mobile_number_div">
                    ...
                    <input type="text" name="mobile_number" class="form-control" id="mobile_number" placeholder="Enter Mobile Number" value="{{old('mobile_number',$result->mobile_number)}}">
                  </div>
                  <div class="form-group col-sm-4 col-xs-4" id="landline_number_div">
                    <label for="landline_number" class="">Landline Number</label>
                    <input type="text" name="landline_number" class="form-control" id="landline_number" placeholder="Enter Landline Number" value="{{old('landline_number',$result->landline_number)}}">
                  </div>
                  <div class="form-group col-sm-4 col-xs-4" id="email_id_div">
                    <label for="email_id" class="required">Email Id</label>
                    <input type="text" name="email_id" class="form-control" id="email_id" placeholder="Enter Email Id" value="{{old('email_id',$result->email_id)}}">
                  </div>
                  <div class="form-group col-sm-4 col-xs-4" id="fax_div" >
                    <label for="fax" class="">Fax</label>
                    <input type="text" name="fax" class="form-control" id="fax" placeholder="Enter Fax" value="{{old('fax',$result->fax)}}">
                  </div>

                  @if(!empty($isProfileUpdate))
                  <input type="hidden" name="hud_id" id="hud_id" value="{{$result->hud_id}}">
                  ...
                  <div class="form-group col-sm-4 col-xs-4" id="hud_div">
                    ...
                    <select name="hud_id" id="hud_id" class="form-control">
                      <option value="">-- Select HUD -- </option> @foreach($huds as $key => $value) <option value="{{$value->id}}" {{SELECT($value->id,old('hud_id',$result->hud_id))}}>{{$value->name}}</option> @endforeach
                    </select>
                  </div>
                  @endif
                  ...
                  <div class="form-group col-sm-4 col-xs-4" id="block_div">
                    ...
                    <select name="block_id" id="block_id" class="form-control">
                      <option value="">-- Select Block -- </option> @foreach($blocks as $key => $value) <option value="{{$value->id}}" {{SELECT($value->id,old('block_id',$result->block_id))}}>{{$value->name}}</option> @endforeach
                    </select>
                  </div>  
                  @endif
                  ...
                  <div class="form-group col-sm-4 col-xs-4" id="phc_div">
                    ...
                    <select name="phc_id" id="phc_id" class="form-control">
                      <option value="">-- Select PHC -- </option> @foreach($phc as $key => $value) <option value="{{$value->id}}" {{SELECT($value->id,old('phc_id',$result->phc_id))}}>{{$value->name}}</option> @endforeach
                    </select>
                  </div>
                  @endif
                  ...
                  <div class="form-group col-sm-4 col-xs-4" id="hsc_div">
                    ...
                    <select name="hsc_id" id="hsc_id" class="form-control">
                      <option value="">-- Select HSC -- </option> @foreach($hsc as $key => $value) <option value="{{$value->id}}" {{SELECT($value->id,old('hsc_id',$result->hsc_id))}}>{{$value->name}}</option> @endforeach
                    </select>
                  </div>
                  @endif
                  @endif
               ...
               @if(!empty($isProfileUpdate))
                ...
                @endif 
                  ...
                  <!--<div class="row pt-3 col-md-12">
                    <div class="form-group col-sm-4 col-xs-4">
                      <label for="image_url" class="">Contact Image </label> @if($result->image_url) <br>
                      <img src="{{fileLink($result->image_url)}}" height="100" width="100" /> @else <br>
                      <span>No Image Uploaded.</span> @endif
                    </div>
                  </div> -->
                </div>
                  <hr>
